    ...
    <div className="space-y-6">
      <div className="bg-cyber-dark border border-cyber-border rounded-lg p-6">
        ...
        <p className="text-gray-400 mb-4">
          Multi-source threat intelligence aggregation with enrichment and correlation from NVD, CVE databases, and OSINT feeds.
        </p>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div className="bg-yellow-900/20 border border-yellow-700 rounded-lg p-4">
              <p className="text-yellow-400 text-2xl font-bold">{cves.length}</p>
          ...
          <div className="bg-cyber-dark border border-cyber-border rounded-lg p-6 space-y-4">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-lg font-semibold text-white">Recent CVE Threats</h3>
            </div>

            <div className="space-y-3 max-h-96 overflow-y-auto">
              {cves.slice(0, 5).map((cve, idx) => {
                const bgColor = cve.severity === "critical" ? "bg-red-900/20 border-red-700" : cve.severity === "high" ? "bg-orange-900/20 border-orange-700" : "bg-yellow-900/20 border-yellow-700";
                const textColor = cve.severity === "critical" ? "text-red-400" : cve.severity === "high" ? "text-orange-400" : "text-yellow-400";
                return (
                  <div key={idx} className={`${bgColor} border rounded p-4`}>
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <p className="text-white font-semibold">{cve.id}</p>
                        <p className="text-gray-400 text-xs">{cve.published_date}</p>
                      </div>
                      <span className={`${textColor} font-bold text-sm`}>{cve.severity}</span>
                    </div>
                    <p className="text-gray-300 text-sm mb-2">{cve.description.substring(0, 120)}...</p>
                    <p className="text-gray-500 text-xs">CVSS: {cve.cvss_score}</p>
                  </div>
                );
              })}
            </div>
