...
export default function NetworkSecurityTab() {
  ...
  const severityData = scanResults
    ? [
        { name: "Critical", value: scanResults.critical_count || 0, color: "#ff0055" },
        { name: "High", value: scanResults.high_count || 0, color: "#ffaa00" },
        { name: "Medium", value: scanResults.medium_count || 0, color: "#00ff88" },
      ]
    : [
    ...
    <div className="space-y-6">
      <div className="bg-cyber-dark border border-cyber-border rounded-lg p-6">
        ...
        <p className="text-gray-400 mb-4">
          Automated network discovery and vulnerability scanning using Python with Nmap integration.
        </p>
        ...
        <div className="mb-4">
          <label className="block text-gray-400 text-sm mb-2">Target Host</label>
          <input
            type="text"
            value={targetHost}
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="bg-cyan-900/20 border border-cyan-700 rounded-lg p-4">
              <p className="text-cyan-400 text-2xl font-bold">{scanResults.ports?.length || 0}</p>
              <p className="text-gray-400 text-sm">Open Ports</p>
            </div>
            <div className="bg-red-900/20 border border-red-700 rounded-lg p-4">
              <p className="text-red-400 text-2xl font-bold">{scanResults.critical_count || 0}</p>
              <p className="text-gray-400 text-sm">Critical Issues</p>
            </div>
            <div className="bg-orange-900/20 border border-orange-700 rounded-lg p-4">
              <p className="text-orange-400 text-2xl font-bold">{scanResults.high_count || 0}</p>
              <p className="text-gray-400 text-sm">High Issues</p>
            </div>
            <div className="bg-green-900/20 border border-green-700 rounded-lg p-4">
              <p className="text-green-400 text-2xl font-bold">
                {(scanResults.critical_count || 0) + (scanResults.high_count || 0) + (scanResults.medium_count || 0)}
              </p>
          ...
          {scanResults.ports && scanResults.ports.length > 0 && (
            <div className="bg-cyber-dark border border-cyber-border rounded-lg p-6 space-y-4">
              <h3 className="text-lg font-semibold text-white">Discovered Services</h3>
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {scanResults.ports.slice(0, 10).map((port, idx) => (
                  <div key={idx} className="bg-cyber-border rounded p-3 text-sm border border-cyan-700">
                    <div className="flex justify-between items-center">
                      <div>
                        <p className="text-cyan-400 font-semibold">Port {port.port}/{port.protocol}</p>
                        <p className="text-gray-300">{port.service}</p>
                      </div>
                      <span className={`text-xs font-bold px-2 py-1 rounded ${port.state === 'open' ? 'bg-red-900/30 text-red-400' : 'bg-gray-700 text-gray-300'}`}>
                        {port.state}
                      </span>
                    </div>
                  </div>
